...
random_sample <- read_rds(here::here("Data", "random_sample.rds"))
crsp <- read_rds(here::here("Data", "final_crsp.rds"))
...
models_1_2 <- function(returns) {
  ...
  reg1 <- lm(dep ~ mkt + smb + hml + umd, 
             data = as.data.frame(returns_2[1:250, ]))
  reg2 <- lm(dep ~ mkt + peer + smb + hml + umd,
             data = as.data.frame(returns_2[1:250, ]))
...
models_3_6 <- function(returns) {
  ...
  # covariate matrix with a market, and peer return, and ffc factors
  MKT_PEER <- cbind(returns[, c("mkt", "smb", "hml", "umd")], 
  ...
  # run model 7 - regularization with CV on mkt, ff factors, and all firms
  mod4 <- mods_3_5(Y, X, folds, c(rep(1, ncol(X))), mod = 4)
  
  # run model 8 - regulation with CV on mkt and all firms, forcing inclusion of market and factors
  mod5 <- mods_3_5(Y, X, folds, c(rep(1, ncol(X) - 4), rep(0, 4)), mod = 5)
...
models_7_8 <- function(returns) {
  ...
  returns_2 <- as.data.frame(cbind(returns[, c("dep", "mkt", "smb", "hml", "umd")], 
  ...
  glmnet.mod7 <- train(dep ~ mkt + peer + smb + hml + umd,
...
simulation <- function(i) {
  ...
  if (flag == 1) {
    ...
    returns <- crsp %>% filter(permno %in% firms & date >= d1 & date <= d2) %>% 
      ...
      # bring in market return variable and ffc factors
      cbind(sp500 %>% filter(caldt >= d1 & caldt <= d2) %>% select(log_sp500, smb, hml, umd)) %>% 
      # give sensible variable names
      setNames(c("dep", paste("comp", 1:(length(firms) - 1), sep = "_"), "mkt", "smb", "hml", "umd"))
...
write_csv(sims, path = here::here("Output", "simulations_final_ff.csv"))
saveRDS(sims, file = here::here("Output", "sims_ff.rds"))
